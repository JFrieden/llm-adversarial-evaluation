...
def get_claude_response(prompt, model="claude-3-haiku-20240307", max_retries=3):
    """
    Send a message to Claude with retry logic and more robust error handling.
...
def process_attack(attack_type, prompt, attack_name, attack_function, model="claude-3-haiku-20240307"):
    attack_prompt = attack_function(prompt)
    response = get_claude_response(attack_prompt, model)
...
output_file = f"claude_campaign/{target_model}_attack_responses_{timestamp}.csv"
